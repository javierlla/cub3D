...
/*   Updated: 2024/10/19 15:34:45 by uxmancis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int	get_texture_color(t_cub *cub, int texNum, int texX, int texY)
{
	...
	{
		printf("Error: Coordenadas de textura fuera de ");
		printf("límites: texX=%d, texY=%d\n", texX, texY);
		return (0);
	}
	colour = *(unsigned int *)(cub->textures_data[texNum]
			+ (texY * cub->line_length + texX * (cub->bits_per_pixel / 8)));
	return (colour);
}
...
int	load_texture(t_cub *cub, int index, char *texture_path)
{
    ...
    //printf("%p\n",  cub->textures[index]);
	cub->textures[index] = mlx_xpm_file_to_image(cub->mlx->mlx_ptr,
